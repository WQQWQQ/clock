...
.controller('ClockCtrl', function($scope,$icu,$storage,$ionicActionSheet,$rootScope,$data,$ionicPopup) {
  ...
  $scope.param={
    ...
    $scope.register=function () {
      try{
        window.JMessage.register("wqq", "123", function (data) {

          $scope.param.msg="注册成功"+JSON.stringify(data);
          window.JMessage.register("lj", "123", function (data) {
            $scope.param.msg="注册成功"+JSON.stringify(data);
            $scope.$apply();
          }, function (data) {
            $scope.param.msg="注册失败"+JSON.stringify(data);
            $scope.$apply();
          });
        }, function (data) {
          $scope.param.msg="注册失败"+JSON.stringify(data);
        ...
        $scope.param.msg=e.toString();
      }
    };
  $scope.login=function () {
    window.JMessage.login('wqq', '123',
      function(data) {
        alert("登录成功"+JSON.stringify(data));
        window.JMessage.getMyInfo(function () {
          alert("我的信息"+JSON.stringify(data));
        }, function (data) {
          alert(JSON.stringify(data));
        })
      }, function(errorStr) {
        alert("登录失败"+JSON.stringify(errorStr));
      });
  };
  $scope.send=function () {
    window.JMessage.sendSingleTextMessage("lj", "你好", "", function (data) {
